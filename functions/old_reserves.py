...
from functions.get_functions_reserves import (
    ...
    future_date,
    get_start_in_edit_data_one,
    get_hour_buttons_in_edit,
    get_date_buttons,
    get_past_reserves,
    get_start_end_paginate,
...
def process_user_reservations(call, session, bot):
    try:
        ...
        if len(reserves) > 0:
            ...
            row.append(
                btn(
                    text=get_text(BotText.PAST_BUTTON, user.language),
                    callback_data="past_reservations_0",
                )
            )
            key.row(*row)
        else:
...
def process_edit_specific_reservation(call, session, bot):
    try:
        ...
        date = (
            reserve.date if user.language == "en" else gregorian_to_jalali(reserve.date)
        )
...
def process_edit_specific_room(call, session, bot):
    try:
        ...
        weekday = weekday if user.language == "en" else day_in_persian[weekday]
        txt = get_text(BotText.EDIT_ROOM_TEXT, user.language).format(
            date=date,
            weekday=weekday,
            start_time=reserve.start_time,
            end_time=reserve.end_time,
...
def process_edit_specific_hours(call, session, bot):
    try:
        ...
        weekday = weekday if user.language == "en" else day_in_persian[weekday]
        txt = get_text(BotText.EDIT_HOURS_TEXT, user.language).format(
...
def change_status_as_selection(call, session, bot):
    try:
        ...
        dt_time = dt.strptime(
            f"{reserve.date} {get_end_time(str_time)}", "%Y-%m-%d %H:%M"
        )
        ...
        if db_status == CONFIRMED and str_time == s_time:
            ...
            db_status == CONFIRMED and get_end_time(s_time) == e_time and dt_time > now
        ):
...
def process_set_edit_hours(call, session, bot):
    try:
        ...
        if reserve.status == FIRST or reserve.status == SECOND:
            ...
            weekday = weekday if user.language == "en" else day_in_persian[weekday]
            t = get_text(BotText.CONFIRM_RESERVATION_TEXT, user.language).format(
                date=date,
                weekday=weekday,
                room_name=room_name,
                start_time=reserve.start_time,
...
def process_delete_specific_reservation(call, session, bot):
    try:
        ...
        weekday = weekday if user.language == "en" else day_in_persian[weekday]
        txt = get_text(BotText.DELETE_SUCCESS_TEXT, user.language).format(
            date=date,
            weekday=weekday,
            room_name=room_name,
            start_time=reserve.start_time,
...
def process_past_reservations(call_page, session, bot):
    try:
        ...
        start_idx, end_idx, paginated_reserves = get_start_end_paginate(
            page, past_reserves
        )
        txt_2 = ""
        for reserve in paginated_reserves:
            ...
            weekday = weekday if user.language == "en" else day_in_persian[weekday]
            txt_2 += (
                get_text(BotText.ADD_TIME_SECOND_STATUS, user.language).format(
                    date=date,
                    weekday=weekday,
                    room_name=room_name,
                    start_time=reserve.start_time,
        ...
        txt, key = get_txt_markup_in_past_reservations(past_reserves, txt_2, user)
        if page > 0:
            key.add(
                btn(
                    text=get_text(BotText.PREVIOUS_BUTTON, user.language),
        ...
        if end_idx < len(past_reserves):
            key.add(
                btn(
                    text=get_text(BotText.NEXT_BUTTON, user.language),
                    callback_data=f"past_reservations_{page + 1}",
                )
            )
        txt = change_num_as_lang(txt, user.language)
