...
def process_ok_reservation(call, session, bot):
    try:
        ...
        if book:
            ...
            txt = ""
            if now > end_time:
                bot.answer_callback_query(call.id, get_text(BotText.INVALID_END_RESERVATION, user.language),
                                          show_alert=True)
                return
            if now < start_time:
                txt = get_text(BotText.FUTURE_MEETING_START, user.language).format(start_time=book.start_time)
                txt = change_num_as_lang(txt, user.language)
            elif now < end_time:
                txt = get_text(BotText.FUTURE_MEETING_END, user.language).format(start_time=book.end_time)
                txt = change_num_as_lang(txt, user.language)
            bot.edit_message_text(chat_id=int(user.chat_id), message_id=msg_id, text=txt)
        else:
            bot.answer_callback_query(call.id, get_text(BotText.RESERVE_NOT_EXISTS, user.language), show_alert=True)
    except Exception as e:
...
def process_cancel_reservation(call, session, bot):
    try:
        ...
        if book:
            ...
            start_time = get_time_as_tehran_tz(book, book.start_time)
            if now > end_time:
                bot.answer_callback_query(call.id, get_text(BotText.INVALID_CANCEL_RESERVATION, user.language),
                                          show_alert=True)
                return
            txt = ""
            if now < start_time:
...
def cancel_after_start_time(session, book_user, now):
    try:
        ...
        session.commit()
        return get_text(BotText.CANCEL_FIXED_TIME, user.language).format(str_time=str_time)
    except SQLAlchemyError as e:
    ...
    try:
        ...
        if reserve:
            ...
            if now < end_time:
                ...
                session.commit()
                txt = get_text(BotText.CHECKOUT_FIXED, user.language).format(str_time=str_time)
                txt = change_num_as_lang(txt, user.language)
                bot.edit_message_text(chat_id=int(user.chat_id), message_id=msg_id, text=txt)
            else:
                bot.answer_callback_query(call.id, get_text(BotText.INVALID_CHECKOUT_RESERVATION, user.language),
                                          show_alert=True)
        else:
            bot.answer_callback_query(call.id, get_text(BotText.RESERVE_NOT_EXISTS, user.language), show_alert=True)
    except Exception as e:
