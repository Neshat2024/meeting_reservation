...
from functions.get_functions_reserves import (
    get_room_name,
    get_room_date_as_call,
    create_date_buttons,
    get_date_in_db,
    get_data_in_process_button,
    get_reservation_in_confirm,
    get_hour_buttons,
    get_date_query_in_add_time,
    get_reserved_hours,
    get_reserved_hours_as_query,
    get_end_time,
    set_end_time_in_process_start,
...
def process_select_room(call, session, bot):
    try:
        ...
        txt = get_text(BotText.ROOM_SELECTION_TEXT, user.language).format(
            date=date, weekday=weekday
        )
        txt = change_num_as_lang(txt, user.language)
        key = add_room_buttons(call, session)
        bot.edit_message_text(chat_id=ch_id, message_id=msg, text=txt, reply_markup=key)
    except Exception as e:
...
def add_room_buttons(call, session):
    ...
    user = get_user(call, session)
    markup.add(
        btn(text=get_text(BotText.BACK_BUTTON, user.language), callback_data="backdate")
    )
    return markup
...
def process_hour_selection(call, session, bot):
    try:
        ...
        date = date if user.language == "en" else gregorian_to_jalali(date)
        weekday = weekday if user.language == "en" else day_in_persian[weekday]
        txt = get_text(BotText.HOUR_SELECTION_TEXT, user.language).format(
            date=date, weekday=weekday, room_name=room_name
        )
        txt = change_num_as_lang(txt, user.language)
...
def delete_status_select_end(call, session):
    try:
        ...
        all_reserves = (
            ...
            .filter_by(user_id=user.id, date=date, room_id=room)
            .all()
        )
...
def create_hour_buttons(call, session):
    ...
    markup.add(
        btn(
            ...
            callback_data=f"confirm-hours_{room}_{date}",
        )
    )
    markup.add(
        btn(
            text=get_text(BotText.BACK_BUTTON, user.language),
            callback_data=f"room_{date}",
        )
    )
    return markup
    ...
    try:
        ...
        if date_in_db and date_in_db.status == FIRST:
            txt = get_text(BotText.ADD_TIME_FIRST_STATUS, user.language).format(
                ...
                weekday=weekday,
                room_name=room_name,
                start_time=date_in_db.start_time,
            ...
            txt = get_text(BotText.ADD_TIME_DEFAULT, user.language).format(
                date=date, weekday=weekday, room_name=room_name
            )
        txt = change_num_as_lang(txt, user.language)
        key = create_hour_buttons(call, session)
        ...
        bot.answer_callback_query(
            call.id,
            get_text(BotText.INVALID_TIME_ALERT, user.language),
            show_alert=True,
        )
    except Exception as e:
    ...
    try:
        ...
        for hour in hours:
            if hour in reserved_hours:
                bot.answer_callback_query(
                    call.id,
                    get_text(BotText.INVALID_RESERVED_TIMES, user.language),
                    show_alert=True,
                )
                break
        else:
            ...
            if s_in_min < e_in_min and (ok_duration or is_admin):
                ...
                bot.answer_callback_query(
                    call.id,
                    change_num_as_lang(
                        get_text(BotText.INVALID_DURATION, user.language), user.language
                    ),
                    show_alert=True,
                )
    except SQLAlchemyError as e:
...
def process_end_hour(call, session, reserve_bot):
    try:
        reserve, bot = reserve_bot
        user = get_user(call, session)
        room, user_id, date, selected_time, e_time = get_data_in_process_button(
            call, session
        )
        ...
        if dt_time > now_time:
            ...
            bot.answer_callback_query(
                call.id,
                get_text(BotText.INVALID_PAST_TIMES, user.language),
                show_alert=True,
            )
    except SQLAlchemyError as e:
...
def process_confirm_selection(call, session, bot):
    try:
        ...
        if reserve and reserve.status:
            ...
            txt = change_num_as_lang(txt, user.language)
            bot.edit_message_text(chat_id=user.chat_id, message_id=msg_id, text=txt)
        else:
            bot.answer_callback_query(
                call.id,
                get_text(BotText.INCOMPLETE_HOURS_ALERT, user.language),
                show_alert=True,
            )
    except SQLAlchemyError as e:
...
def process_who_reserved(call, session, bot):
    try:
        ...
        reserves = (
            session.query(Reservations)
            .filter_by(date=date, room_id=room, status=CONFIRMED)
            .all()
        )
        reserved_times = [
            (row.start_time, row.end_time, row.user_id) for row in reserves
        ]
